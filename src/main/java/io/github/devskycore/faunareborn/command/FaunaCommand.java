...
public final class FaunaCommand implements BasicCommand {
    ...
    public FaunaCommand(FaunaRebornPlugin plugin, FaunaMainGui mainGui, PluginGuiConfigService guiConfigService) {
        PermissionService permissionService = new PermissionService();

        HelpCommand helpCommand = new HelpCommand(permissionService);
        CommandRegistry builtRegistry = new CommandRegistry(
                CommandRegistry.defaults(
                        helpCommand,
                        new VersionCommand(plugin),
                        new AboutCommand(),
                        new EntitiesCommand(guiConfigService),
                        new ReloadCommand(plugin.reloadService()),
                        new GuiCommand(mainGui)
                ),
                permissionService
        );
